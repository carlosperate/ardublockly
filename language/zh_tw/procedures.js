...
Blockly.Language.procedures_defnoreturn = {
  ...
  init: function() {
    this.setColour(290);
    var name = Blockly.Procedures.findLegalName('流程', this);
    this.appendTitle(new Blockly.FieldTextInput(name, Blockly.Procedures.rename), 'NAME');
    this.appendInput('執行', Blockly.NEXT_STATEMENT, 'STACK');
...
Blockly.Language.procedures_defreturn = {
  ...
  init: function() {
    ...
    this.appendTitle(new Blockly.FieldTextInput(name, Blockly.Procedures.rename), 'NAME');
    this.appendInput('執行', Blockly.NEXT_STATEMENT, 'STACK');
    this.appendInput('回傳', Blockly.INPUT_VALUE, 'RETURN', null);
